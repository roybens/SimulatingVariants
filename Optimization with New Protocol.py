...
def run(objective_names):
    evaluator = vcl_ev.Vclamp_evaluator_HMM('./params_na12mut8st.csv', 'A427D', 'na12mut8st', objective_names=objective_names)
    gen_counter = 0
    best_indvs = []
    cp_freq = 1
    ...
    deap_opt = bpop.optimisations.DEAPOptimisation(evaluator, offspring_size=1000, hof = hof, map_function=pool.map)
    cp_file = './cp.pkl'
