...
def activationNa12(command):
    ...
    soma.nseg   = 1
    soma.insert('na12mut')     #insert mechanism
    soma.ena    = 55
    h.celsius   = 33        # temperature in celsius
    ...
    step        = 10         # voltage clamp increment, the user can 
    st_cl       = -120       # clamp start, mV 
    end_cl      = 40        # clamp end, mV
            ...
            while (h.t<h.tstop): # runs a single trace, calculates peak current
                ...
                if ((h.t>5)and(h.t<=10)):       # evaluate the peak 
                    if(abs(dens)>abs(pre_i)):
                        cond_tr = soma.thegna_na12mut   # updates the peak conductance
...
def inactivationNa12(command):
    ...
    # clamping parameters
    dur         = 500        # clamp duration, ms
    step        = 10          # voltage clamp increment
    st_cl       = -120       # clamp start, mV
    end_cl      = 40          # clamp end, mV
    v_cl        = -120       # actual voltage clamp, mV
...
##################
# Recovery of Inactivation Tau Na 1.2 
##################
 
# Args: String command
# Calls recInactTauNa12 plotting function when command == "plotRecInactTau"
# Calls recInactTauNa12 data generating function when command == "genRecInactTau"
# genRecInactTau Returns vector for recovery of inactivation tau
def recInactTauNa12(command):
    ...
    # for RecInactTau
    print(np.linspace(min_inter, max_inter,num=num_pts))
...
def find_tau_inact(inact_i,ax=None):
    ...
    for i in range(len(inact_i)):
        ...
        min_val, mindex = min((val, idx) for (idx, val) in enumerate(raw_data))
        data = raw_data[mindex:mindex+100]
        if ax is not None:
            if all_tau_sweeps[i] >= -30:
                ax.plot(data,label=str(all_tau_sweeps[i]))
                ax.legend()
...
def plot_act_inact_mut(new_params,wt_data):
    ...
    act,act_x = activationNa12("genActivation")
