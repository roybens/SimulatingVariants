...
class Inactivation:
    def __init__(self, soma_diam=50, soma_L=63.66198, soma_nseg=1, soma_cm=1, soma_Ra=70,
                 channel_name='na16mut', soma_ena=55, h_celsius=33, v_init=-120, h_dt=0.025, ntrials=range(30),
...
class RFI:
    def __init__(self, soma_diam=50, soma_L=63.66198, soma_nseg=1, soma_cm=1, soma_Ra=70,
                 channel_name='na16mut', soma_ena=55, h_celsius=33, v_init=-90, h_dt=0.1, ntrials=30,
...
class Ramp:
    def __init__(self, soma_diam=50, soma_L=63.66198, soma_nseg=1, soma_cm=1, soma_Ra=70,
                 channel_name='na16mut', soma_ena=55, h_celsius=33, v_init=-120, t_init=30,
...
class UDB20:
    def __init__(self, soma_diam=50, soma_L=63.66198, soma_nseg=1, soma_cm=1, soma_Ra=70,
                 channel_name='na16mut', soma_ena=55, h_celsius=33, v_init=-120, v_held=-70,
...
class RFI_dv:
    def __init__(self, ntrials=30, recordTime=500,
                 ...
                 channel_name='na16mut', soma_ena=55, h_celsius=33, v_init=-120, h_dt=0.01,
...
def update_params(vc_params):
    nrn_h = activationna16("geth")
    params = list(vc_params.keys())
    for p in params:
        nrn_h(p + '_na16mut =' + str(vc_params[p]))
...
def plot_act_inact_mut(new_params, wt_data):
    ...
    act, act_x, act_i = activationna16("genActivation")
    act = list(act.to_python())
    act_x = list(act_x)
    inact, inact_x, inact_i = inactivationna16("genInactivation")
...
# vclamp_params = {'tha_na16mut':-15,'qa_na16mut':7.2,'thinf_na16mut':-45,'qinf_na16mut':7}
# plot_act_inact_mut(vclamp_params,wt_act_inact)
# plot_act_inact_wt()
# recInactTauna16("plotRecInact")
...
if __name__ == "__main__":
    ...
    if args.function == 1:
        genAct = Activation(channel_name='na16mut8st')
        genAct.genActivation()
        genAct.plotAllActivation()

    elif args.function == 2:
        genInact = Inactivation(channel_name='na16mut8st')
        genInact.genInactivation()
        genInact.plotAllInactivation()


    elif args.function == 3:
        genRFI = RFI(channel_name='na16mut8st')
        ...
        # na (na8st.mod) (corresponding na16)
