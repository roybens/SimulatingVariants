...
class Vclamp_evaluator(bpop.evaluators.Evaluator):
    ...
    def __init__(self, scaled, other_widgets, ratio_widgets, diff_widgets):

        eh12.set_channel(other_widgets[2].value)
   ...
   "Tau 0mV (%WT)", "Persistent (%WT)", "Peak Amp (%WT)"]
        ...
        self.V_half_Act_diff_goal = diff_widgets["DV1/2 Act"].value
        self.V_half_inact_diff_goal = diff_widgets["DV1/2 SSI"].value
        # slopes come in the 100 scale since it's a ratio, so we have to divide by 100
        self.slope_Act_ratio_goal = ratio_widgets["GV Slope (%WT)"].value
        self.slope_inact_ratio_goal = ratio_widgets["SSI Slope (%WT)"].value
        self.tau0_ratio_goal = ratio_widgets["Tau 0mV (%WT)"].value
        self.per_cur_ratio_goal = ratio_widgets["Persistent (%WT)"].value
        self.peak_amp_ratio_goal = ratio_widgets["Peak Amp (%WT)"].value
...
cur_log_file = 'optimization_output.txt'
...
def run_opt(other_widgets, ratio_widgets, diff_widgets):
    offspring_size = other_widgets[0].value
    num_generations = other_widgets[1].value
    mutant_name = other_widgets[3].value

    print(offspring_size)
    print(num_generations)

    evaluator = Vclamp_evaluator(False, other_widgets, ratio_widgets, diff_widgets)
    ...
    #deap_opt = bpop.optimisations.DEAPOptimisation(evaluator, offspring_size=offspring_size, hof = hof)
