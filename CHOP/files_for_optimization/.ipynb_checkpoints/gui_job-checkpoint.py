...
ith open('opt_config.pickle', 'rb') as handle:
    config_dict = pickle.load(handle)

offspring_size = config_dict['# of offsprings:']
num_generations = config_dict['# of generations:']
output_log_file_name = 'optimization_output.txt'
param_range_file = "./csv_files/param_stats_wide_na12.csv"
mutant_name = config_dict['mutant name:']
...
class Vclamp_evaluator(bpop.evaluators.Evaluator):
    ...
    def __init__(self, scaled):
        ...
        eh12.set_channel(config_dict['channel name:'])
        ...
        self.V_half_Act_diff_goal = config_dict["DV1/2 Act"]
        self.V_half_inact_diff_goal = config_dict["DV1/2 SSI"]
        # slopes come in the 100 scale since it's a ratio, so we have to divide by 100
        self.slope_Act_ratio_goal = config_dict["GV Slope (%WT)"]
        self.slope_inact_ratio_goal = config_dict["SSI Slope (%WT)"]
        self.tau0_ratio_goal = config_dict["Tau 0mV (%WT)"]
        self.per_cur_ratio_goal = config_dict["Persistent (%WT)"]
        self.peak_amp_ratio_goal = config_dict["Peak Amp (%WT)"]
