...
evaluator = vcl_ev.Vclamp_evaluator_HMM('./csv_files/params_na12mut8st_mod.csv', 'Basis', 'na12mut8st', 'na16', objective_names=['inact'])
...
deap_opt = bpop.optimisations.DEAPOptimisation(evaluator, offspring_size=5, hof = hof, map_function=pool.map)  # CHANGE offspring_size

cp_file = './cp.pkl'

start_time = time.time()
pop, hof, log, hst = deap_opt.run(max_ngen=2, cp_filename=cp_file)  # CHANGE max_ngen
end_time = time.time()
print(end_time - start_time)

print("Best params: " + str(best_indvs[-1]))
print("[inact_err]: " + str(evaluator.evaluate_with_lists(best_indvs[-1])))
